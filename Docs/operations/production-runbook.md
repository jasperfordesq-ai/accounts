...
Run `scripts\verify-release-evidence-workspace.ps1 -WorkspaceDirectory D:\accounts-release-evidence -CommitSha <release-commit-sha> -GitHubActionsRunUrl <ci-run-url>` and retain `release-evidence-workspace-verification-report.json`, `release-evidence-reviewer-completion.json`, plus `release-evidence-verifier-output.txt` before sending the workspace to reviewers; the verifier must prove the workspace is well-formed, the reviewer completion ledger still has all six gates pending, and release evidence stays blocked until named human sign-off is complete.
