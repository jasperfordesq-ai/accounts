...
The helper copies the six templates, pre-fills only release identity and machine-derived evidence fields, writes `release-evidence-workspace-manifest.json` and `release-evidence-reviewer-index.md`, and leaves all reviewer identity, decisions, signatures, external validation references and human acceptance cells blank.
Run `scripts\verify-release-evidence-workspace.ps1 -WorkspaceDirectory D:\accounts-release-evidence -CommitSha <release-commit-sha> -GitHubActionsRunUrl <ci-run-url>` and retain `release-evidence-workspace-verification-report.json` plus `release-evidence-verifier-output.txt` before sending the workspace to reviewers; the verifier must prove the workspace is well-formed and still blocked until named human sign-off is complete.
