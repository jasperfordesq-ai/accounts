...
The helper copies the six templates, pre-fills only release identity and machine-derived evidence fields, writes `release-evidence-workspace-manifest.json`, and leaves all reviewer identity, decisions, signatures, external validation references and human acceptance cells blank.
Default CI also uploads the same prepared files as the `release-evidence-reviewer-workspace` artifact after the machine evidence pack is verified, including a failed `release-evidence-report.json` that lists the remaining named human evidence blockers.
