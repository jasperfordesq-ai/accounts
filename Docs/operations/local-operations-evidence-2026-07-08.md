...
| Release evidence verifier parser and draft failure | PowerShell parser check plus `powershell -ExecutionPolicy Bypass -File scripts\verify-release-evidence.ps1 -EvidenceDirectory .\Docs\release-evidence -ReportPath <temp>\accounts-release-evidence-draft-external-report.json` | Passed: parser accepted the script; blank templates fail as expected and write a failed JSON report with canonical required coverage for source-law source IDs, `micro-ltd`, `small-abridged-ltd`, `dac-small`, `clg-charity`, `medium-audit-required`, external ROS/iXBRL scenario rows, manual handoff scenario/path rows, the seven route codes, and the required release artifacts |
| Release evidence verifier completed-template smoke | Generated temporary filled copies of the six release evidence templates outside the repo, then ran `powershell -ExecutionPolicy Bypass -File scripts\verify-release-evidence.ps1 -EvidenceDirectory <temp>\accounts-release-evidence-filled-* -ReportPath <temp>\release-evidence-report.json` | Passed: verifier accepted completed source-law review, visual QA, external ROS/iXBRL validation, qualified-accountant acceptance, manual handoff acceptance, and monitoring provider confirmation evidence with no failures and retained source-law source, canonical scenario, manual handoff, route, and artifact coverage in the JSON report |
