...
| Release evidence verifier parser and draft failure | PowerShell parser check plus `powershell -ExecutionPolicy Bypass -File scripts\verify-release-evidence.ps1 -EvidenceDirectory .\Docs\release-evidence -ReportPath <temp>\accounts-release-evidence-draft-external-report.json` | Passed: parser accepted the script; blank templates failed as expected and wrote a failed JSON report with 67 issues plus canonical required coverage for `micro-ltd`, `small-abridged-ltd`, `dac-small`, `clg-charity`, `medium-audit-required`, external ROS/iXBRL scenario rows, the seven route codes, and the required release artifacts |
| Release evidence verifier completed-template smoke | Generated temporary filled copies of the four release evidence templates outside the repo, then ran `powershell -ExecutionPolicy Bypass -File scripts\verify-release-evidence.ps1 -EvidenceDirectory <temp>\accounts-release-evidence-filled-external -ReportPath <temp>\accounts-release-evidence-filled-external-report.json` | Passed: verifier accepted completed visual QA, external ROS/iXBRL validation, qualified-accountant acceptance, and monitoring provider confirmation evidence with no failures and retained canonical scenario/route/artifact coverage in the JSON report |
...
| Frontend visual smoke evidence report contract | `node --test tests/visual-smoke-plan.test.mjs tests/visual-smoke-artifacts.test.mjs tests/production-readiness-contract.test.mjs` from `frontend` | Passed: 50 tests proving the 28-screenshot matrix, visual-smoke-evidence-report generation, duplicate coverage rejection, parser invariants, and 514/700 scorecard contract |
| Frontend dependency readiness contract | `node --test --experimental-strip-types tests/production-readiness-contract.test.mjs`; `node scripts/verify-api-client.mjs`; `npm.cmd run test:render -- production-readiness-workbench`; `npm.cmd run test:render -- production-readiness-panel`; `npx.cmd tsc --noEmit --incremental false` from `frontend` | Passed: contract tests including production scorecard invariants, API-client verifier, 2 render tests, and TypeScript validation for the 514/700 scorecard fixtures |
