...
| Frontend visual smoke evidence report contract | `node --test tests/visual-smoke-plan.test.mjs tests/visual-smoke-artifacts.test.mjs tests/production-readiness-contract.test.mjs` from `frontend` | Passed previously: 50 tests proving the 28-screenshot matrix, visual-smoke-evidence-report generation, duplicate coverage rejection, parser invariants, and scorecard contract |
| Frontend dependency readiness contract | `node --test --experimental-strip-types tests/production-readiness-contract.test.mjs`; `node scripts/verify-api-client.mjs`; `npm.cmd run test:render -- production-readiness-workbench production-readiness-panel`; `npx.cmd tsc --noEmit --incremental false` from `frontend` | Passed: contract tests including production scorecard invariants, API-client verifier, 2 render tests, and TypeScript validation for the 539/700 scorecard fixtures |
