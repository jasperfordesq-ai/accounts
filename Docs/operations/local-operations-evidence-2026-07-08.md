...
| Frontend dependency readiness contract | `node --test --experimental-strip-types tests/production-readiness-contract.test.mjs`; `node scripts/verify-api-client.mjs`; `npm.cmd run test:render -- production-readiness-workbench`; `npx.cmd tsc --noEmit --incremental false` from `frontend` | Passed: 41 contract tests including production scorecard invariants, API-client verifier, 1 render test, and TypeScript validation for the 509/700 scorecard fixtures |
