...
| Release evidence verifier parser and draft failure | PowerShell parser check plus `powershell -ExecutionPolicy Bypass -File scripts\verify-release-evidence.ps1 -EvidenceDirectory .\Docs\release-evidence -ReportPath <temp>\accounts-release-evidence-draft-report.json` | Passed: parser accepted the script; blank templates failed as expected and wrote a failed JSON report with 52 issues |
| Release evidence verifier completed-template smoke | Generated temporary filled copies of the three release evidence templates outside the repo, then ran `powershell -ExecutionPolicy Bypass -File scripts\verify-release-evidence.ps1 -EvidenceDirectory <temp>\accounts-release-evidence-filled -ReportPath <temp>\accounts-release-evidence-filled-report.json` | Passed: verifier accepted completed visual QA, qualified-accountant acceptance, and monitoring provider confirmation evidence with no failures |
...
| Frontend dependency readiness contract | `node --test --experimental-strip-types tests/production-readiness-contract.test.mjs`; `node scripts/verify-api-client.mjs`; `npm.cmd run test:render -- production-readiness-workbench` from `frontend` | Passed: 41 contract tests including production scorecard invariants, API-client verifier, and 1 render test |
