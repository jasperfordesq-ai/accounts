...
| Release evidence verifier parser and draft failure | PowerShell parser check plus `powershell -ExecutionPolicy Bypass -File scripts\verify-release-evidence.ps1 -EvidenceDirectory .\Docs\release-evidence -ReportPath <temp>\accounts-release-evidence-draft-report.json` | Passed: parser accepted the script; blank templates failed as expected and wrote a failed JSON report with 52 issues plus canonical required coverage for `micro-ltd`, `small-abridged-ltd`, `dac-small`, `clg-charity`, `medium-audit-required`, the seven route codes, and the required release artifacts |
| Release evidence verifier completed-template smoke | Generated temporary filled copies of the three release evidence templates outside the repo, then ran `powershell -ExecutionPolicy Bypass -File scripts\verify-release-evidence.ps1 -EvidenceDirectory <temp>\accounts-release-evidence-filled-canonical -ReportPath <temp>\accounts-release-evidence-filled-canonical-report.json` | Passed: verifier accepted completed visual QA, qualified-accountant acceptance, and monitoring provider confirmation evidence with no failures and retained canonical scenario/route/artifact coverage in the JSON report |
