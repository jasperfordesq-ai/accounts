...
- That previous slice moved the production scorecard to 669/700, with backend
  statutory/accounting engine at 244/250.
- Verification completed locally:
  - PowerShell parser check for `scripts\verify-release-evidence.ps1` passed.
  - Temporary completed release-evidence pack outside the repo passed with exact
...
Frontend visual QA exact artifact-filename checks:

- This slice tightened `scripts/verify-release-evidence.ps1` so visual QA
  release evidence must name the exact retained artifact files:
  `visual-smoke-manifest.json`, `visual-smoke-evidence-report.json`, and
  `accountant-workbench-evidence-report.json`.
- The visual QA sign-off template now tells reviewers that those visual artifact
  file fields must be exact, so a completed evidence pack cannot silently point
  at renamed or substituted visual evidence.
- That previous slice moved the production scorecard to 671/700, with frontend
  accountant workbench at 178/200.
- Verification completed locally:
  - PowerShell parser check for `scripts\verify-release-evidence.ps1` passed.
  - Temporary completed release-evidence pack outside the repo passed with exact
    visual artifact filenames.
  - A copied visual QA pack with `Visual smoke manifest file` set to
    `wrong-manifest.json` failed with the expected exact visual artifact filename
    verifier error.
  - Backend focused regression passed 3 tests:
    `ReleaseEvidenceVerifier_BlocksIncompleteHumanSignoffEvidence`,
    `ReleaseEvidenceTemplates_CoverHumanVisualAccountantAndProviderSignoffs`,
...
Backend external ROS/iXBRL top-level evidence checks:

- This slice tightened `scripts/verify-release-evidence.ps1` so external
  ROS/iXBRL validation evidence rejects placeholder provider, environment,
  validation run/reference, report, taxonomy package and company/period fields.
- The verifier now also requires `Generated iXBRL artifact name` to name a
  retained `.xhtml`, `.html`, or `.zip` artifact for the release candidate.
...
- Verification completed locally:
  ...
  - Temporary completed release-evidence pack outside the repo passed with real
    external validation top-level evidence fields.
...
- The qualified-accountant acceptance template now tells reviewers those
  ...
  placeholders.
- That previous slice moved the production scorecard to 677/700, with backend
  statutory/accounting engine at 250/250.
- Verification completed locally:
  - PowerShell parser check for `scripts\verify-release-evidence.ps1` passed.
  - Temporary completed release-evidence pack outside the repo passed with real
...
Frontend visual QA top-level evidence checks:

- This slice tightens `scripts/verify-release-evidence.ps1` so visual QA
  sign-off evidence rejects placeholder reviewer name, reviewer role and
  reviewer signature fields before human visual sign-off evidence can pass.
- The visual QA sign-off template now tells reviewers those top-level identity
  fields must be real retained evidence values, not placeholders.
- That previous slice moved the production scorecard to 679/700, with frontend
  accountant workbench at 180/200.
- Verification completed locally:
  - PowerShell parser check for `scripts\verify-release-evidence.ps1` passed.
  - Temporary completed release-evidence pack outside the repo passed with real
    visual reviewer identity and signature fields.
  - A copied visual QA pack with `Reviewer name` set to `accepted` failed with
    the expected real visual QA reviewer name verifier error.
  - Backend focused regression passed 3 tests:
    `ReleaseEvidenceVerifier_BlocksIncompleteHumanSignoffEvidence`,
    `ReleaseEvidenceTemplates_CoverHumanVisualAccountantAndProviderSignoffs`,
    and
...
Frontend accountant-workbench route acceptance pack checks:

- This slice tightens `scripts/verify-release-artifact-pack.ps1` and
  ...
  include the exact route names, route keys, expected accountant decision text
  and per-route acceptance evidence ids for every visual smoke workbench route.
- That previous slice moved the production scorecard to 681/700, with frontend
  accountant workbench at 182/200.
- Verification completed locally:
...
Frontend accountant-workbench route review-status pack checks:
...
- This slice further tightens `scripts/verify-release-artifact-pack.ps1` and
  ...
  generators.
- That previous slice moved the production scorecard to 687/700, with frontend accountant workbench at
  188/200.
- Verification completed locally:
  - PowerShell parser checks for both pack verifiers passed.
...
Frontend visual-smoke top-level coverage pack checks:

- This slice further tightens `scripts/verify-release-artifact-pack.ps1` and
  `scripts/verify-ci-machine-evidence-pack.ps1` so retained
  `visual-smoke-evidence-report.json` evidence must include exact top-level
  themes, viewports, planned viewport dimensions, layout/contrast result counts,
  minimum contrast ratio, retained screenshot byte evidence and per-route
  coverage before the release or CI machine evidence pack can pass.
- That previous slice moved the production scorecard to 689/700, with frontend accountant workbench at
  190/200.
- Verification completed locally:
  - PowerShell parser checks for both pack verifiers passed.
  - Temporary visual evidence packs proved both verifiers reject a missing
    top-level `visual-smoke-evidence-report.json` theme value.
  - Backend focused regression passed 2 tests:
    `ReleaseArtifactPackVerifier_RequiresExactOperationalEvidenceReports` and
    `ProductionReadinessReport_ExposesGoalScorecardMappedToReleaseBlockers`.
  - Frontend contract/API/render/type checks passed:
...
Frontend visual-smoke screenshot-matrix pack checks:

- This slice further tightens `scripts/verify-release-artifact-pack.ps1` and
  `scripts/verify-ci-machine-evidence-pack.ps1` so retained
  `visual-smoke-evidence-report.json` screenshot rows must include the exact
  28 route/theme/viewport matrix, with exact route keys, screenshot filenames,
  expected accountant decision text and `required-review` status for every
  capture.
- That previous slice moved the production scorecard to 691/700, with frontend
  accountant workbench at 192/200.
- Verification completed locally:
  - PowerShell parser checks for both pack verifiers passed.
  - Temporary visual evidence packs proved both verifiers reject a wrong
    dashboard light desktop screenshot filename.
  - Backend focused regression passed 2 tests:
    `ReleaseArtifactPackVerifier_RequiresExactOperationalEvidenceReports` and
    `ProductionReadinessReport_ExposesGoalScorecardMappedToReleaseBlockers`.
  - Frontend contract/API/render/type checks passed:
...
Frontend visual-smoke screenshot checksum pack checks:

- This slice further tightens `scripts/verify-release-artifact-pack.ps1` and
  `scripts/verify-ci-machine-evidence-pack.ps1` so retained
  `visual-smoke-evidence-report.json` screenshot rows must carry a positive
  `byteSize` and canonical `sha256:<64 lowercase hex>` checksum before retained
  visual evidence can pass.
- That previous slice moved the production scorecard to 693/700, with frontend
  accountant workbench at 194/200.
- Verification completed locally:
  - PowerShell parser checks for both pack verifiers passed.
  - Temporary visual evidence packs proved both verifiers reject a malformed
    dashboard light desktop screenshot checksum and a zero-byte screenshot row,
    while the unmodified temp pack did not raise the new checksum/byte errors.
  - Backend focused regression passed 2 tests:
    `ReleaseArtifactPackVerifier_RequiresExactOperationalEvidenceReports` and
    `ProductionReadinessReport_ExposesGoalScorecardMappedToReleaseBlockers`.
  - Frontend contract/API/render/type checks passed:
...
Frontend accountant-workbench route coverage pack checks:
...
Frontend visual-smoke manifest/evidence cross-checks:

- This slice tightens `scripts/verify-release-artifact-pack.ps1` and
  `scripts/verify-ci-machine-evidence-pack.ps1` so
  `visual-smoke-manifest.json` is parsed as evidence and its route audits plus
  screenshot rows must match `visual-smoke-evidence-report.json` for route keys,
  filenames, expected text, review status, byte sizes, checksums and dimensions.
- The production scorecard is now 698/700, with frontend accountant workbench at
  199/200.
- Verification completed locally:
  - PowerShell parser checks for both pack verifiers passed.
  - Temporary visual evidence packs copied the retained PNG set and proved both
    verifiers reject mismatched manifest/evidence screenshot fields.
  - Backend focused regression passed 2 tests:
    `ReleaseArtifactPackVerifier_RequiresExactOperationalEvidenceReports` and
    `ProductionReadinessReport_ExposesGoalScorecardMappedToReleaseBlockers`.
  - Frontend contract/API/render/type checks passed:
...
- This slice adds a `Human evidence closeout path` panel to the accountant-facing
  production readiness workbench. It surfaces the five-step release sequence:
  pick up the prepared `release-evidence-reviewer-workspace`, complete the six
  templates, run `scripts/verify-release-evidence.ps1`, confirm six accepted
...
- This slice adds a compact `Human evidence closeout` card to the dashboard production
  readiness panel, so the first readiness surface also shows the reviewer workspace
  pickup, remaining template, release-evidence verifier, `humanEvidenceCompletion`,
  and final artifact-pack steps.
...
Production readiness closeout contract:

- This slice promotes the five-step human evidence closeout path into the production
  readiness API contract as `humanReleaseEvidenceCloseout`, generated by
  `ProductionReadinessReportService` from the six human release-evidence gates.
- The frontend parser now rejects reports whose closeout path omits, reorders or
  retargets the prepared `release-evidence-reviewer-workspace`, template,
  `scripts/verify-release-evidence.ps1`, `release-evidence-report.json` /
  `humanEvidenceCompletion`, or `scripts/verify-release-artifact-pack.ps1` steps.
  The dashboard and full workbench now render those backend-provided steps instead
  of local hardcoded copies.
...
- `humanReleaseEvidenceCloseout` is now a five-step operator sequence. Step 1 is
  ...
  `release-evidence-reviewer-completion.json`, the retained handoff files and the
  pending human blocker inventory before assigning reviewers.
- The generated reviewer index, production-readiness report verifier, typed
  frontend parser, final release artifact-pack verifier and CI machine-evidence
...
- The production scorecard is now 698/700: architecture/documentation 99/100,
  backend statutory/accounting engine 250/250, frontend accountant workbench 199/200,
