      ...
      {canWrite && !candidate.batchDecisionAvailable && (
        <div className="mt-4 space-y-2 border-t border-[var(--border)] pt-3">
          <label className="block text-xs font-semibold text-[var(--foreground)]" htmlFor={`duplicate-reason-${candidate.transactionId}`}>
            ...
            <textarea id={`duplicate-reason-${candidate.transactionId}`} aria-label={`${unresolved ? "Reviewer decision reason" : "Reason for changing this decision"} for transaction ${candidate.transactionId}`} aria-describedby={`duplicate-reason-help-${candidate.transactionId}`} value={reason} onChange={(event) => onReasonChange(event.target.value)} rows={2} maxLength={1000} className="mt-1.5 w-full resize-y rounded-md border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm font-normal text-[var(--foreground)]" placeholder="Explain the bank-statement evidence and why the incoming row should be retained or excluded." />
          </label>
    ...
    <article className="min-w-0 rounded-md border border-[var(--border)] bg-[var(--surface)] p-3">
      ...
      {canWrite && (
        <div className="mt-3 space-y-2 border-t border-[var(--border)] pt-3">
          ...
          <textarea id={`duplicate-batch-reason-${batch.importBatchId}`} aria-label={`${pending ? "Batch decision reason" : "Reason for reopening this batch"} for batch ${batch.importBatchId}`} aria-describedby={`duplicate-batch-help-${batch.importBatchId}`} value={reason} onChange={(event) => onReasonChange(event.target.value)} rows={2} maxLength={1000} className="w-full resize-y rounded-md border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" placeholder="Explain why the whole source statement is an exact re-import and how it should be treated." />
