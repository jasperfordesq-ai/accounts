...
test("parseProductionReadinessReport accepts the golden corpus evidence-pack contract", () => {
  ...
  assert.equal(parsed.productionScorecard.currentScore, 635);
  ...
  assert.equal(parsed.productionScorecard.categories[1].currentScore, 216);
...
test("parseProductionReadinessReport rejects scorecard totals that do not match categories", () => {
  ...
  assert.throws(
    () => parseProductionReadinessReport(payload),
    /Invalid production readiness report contract: productionScorecard\.currentScore - expected 635, received 491/,
  );
});
