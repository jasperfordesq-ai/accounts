...
test("parseProductionReadinessReport accepts the golden corpus evidence-pack contract", () => {
  ...
  assert.equal(parsed.productionScorecard.currentScore, 645);
  ...
  assert.equal(parsed.productionScorecard.categories[1].currentScore, 224);
...
test("parseProductionReadinessReport rejects scorecard totals that do not match categories", () => {
  ...
  assert.throws(
    () => parseProductionReadinessReport(payload),
    /Invalid production readiness report contract: productionScorecard\.currentScore - expected 645, received 491/,
  );
});
    ...
    categories: [
      ...
      },
      {
        ...
        currentEvidence: [
          ...
          "Visual QA route notes require retained route-specific visual evidence references containing the matching route code before sign-off evidence can pass.",
