...
import { productionScorecardControls } from "../tests/fixtures/production-scorecard-controls.ts";
...
globalThis.fetch = async (url, init) => {
  ...
  const body = requestUrl === "/api/system/production-readiness"
    ...
    : requestUrl.endsWith("/director-loans") && (init?.method ?? "GET").toUpperCase() === "GET"
      ? []
      : requestUrl.includes("/director-loans") && ["POST", "PUT"].includes((init?.method ?? "GET").toUpperCase())
        ? { ...JSON.parse(String(init?.body ?? "{}")), id: 9, periodId: 2 }
        : { ok: true };
...
const dirLoan = {
  ...
  reviewNote: "Reviewed against the retained statutory evidence.",
...
function humanReleaseEvidenceFixture() {
  return [
...
function workbenchVisualAcceptance(routeCode, routeLabel, workflowStages, nextAction) {
  return {
    ...
    screenshotArtifactNames: ["dark-desktop", "dark-mobile", "dark-tablet", "light-desktop", "light-mobile", "light-tablet"].map(
      (suffix) => `${routeCode}-${suffix}.png`,
    ),
