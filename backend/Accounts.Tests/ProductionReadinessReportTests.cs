...
public class ProductionReadinessReportTests
{
    ...
    public async Task ProductionReadinessReport_ExposesGoalScorecardMappedToReleaseBlockers()
    {
        ...
        Assert.Contains(scores["architecture-documentation"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("pending human-evidence blockers", StringComparison.OrdinalIgnoreCase)
        ...
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("concrete URL reachability", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("not-dated effective-date review", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("exact accepted decisions", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("Source-law review platform impact cells", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("exact no change, reflected or blocking", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
        ...
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("sign-off fields", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("External ROS/iXBRL validation", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("real references", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("retained taxonomy package references", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("accepted/remediated warning status", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("accepted scenario decisions", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
        ...
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("exact accepted values", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("External ROS/iXBRL validation top-level evidence", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("rejects placeholder provider", StringComparison.OrdinalIgnoreCase)
        ...
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("Manual handoff scenario and unsupported-path decisions", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("exact accepted reviewer decisions", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("Manual handoff evidence references", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("matching scenario or unsupported-path code", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("Manual handoff evidence rows", StringComparison.OrdinalIgnoreCase)
        ...
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("professional sign-off evidence", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
        ...
        Assert.Contains(scores["security-auth-tenant-platform-guardrails"].CurrentEvidence, evidence =>
            evidence.Contains("verify-release-evidence.ps1", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("UTC timestamps", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("SHA-256", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["security-auth-tenant-platform-guardrails"].CurrentEvidence, evidence =>
            evidence.Contains("releaseCandidate identity", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("CommitSha", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("GitHubActionsRunUrl", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["security-auth-tenant-platform-guardrails"].CurrentEvidence, evidence =>
            evidence.Contains("human release-evidence templates", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("matching hashes", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["security-auth-tenant-platform-guardrails"].CurrentEvidence, evidence =>
        ...
        Assert.Contains(scores["security-auth-tenant-platform-guardrails"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("six unassigned reviewer assignment rows", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("complete per-gate pickup-file guidance", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["security-auth-tenant-platform-guardrails"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("release artifact pack", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["security-auth-tenant-platform-guardrails"].CurrentEvidence, evidence =>
            evidence.Contains("verify-release-artifact-pack.ps1", StringComparison.OrdinalIgnoreCase)
        ...
        Assert.Contains(scores["security-auth-tenant-platform-guardrails"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("six-row reviewer assignment inventory", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("production-readiness-verification-report.json", StringComparison.OrdinalIgnoreCase)
        ...
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("reject accepted-style ambiguous text", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            evidence.Contains("Visual QA release evidence", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("exact visual-smoke manifest", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("accountant workbench evidence report filenames", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            evidence.Contains("Visual QA top-level evidence", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("placeholder reviewer name", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("reviewer role", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("human visual sign-off evidence", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
        ...
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("exact accountant-workbench route acceptance names", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("expected decision text", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("per-route acceptance evidence ids", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            evidence.Contains("Release artifact and CI machine evidence pack verifiers", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("route acceptance labels", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("screenshot-review evidence anchors", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("required-review status", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            evidence.Contains("Release artifact and CI machine evidence pack verifiers", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("route readiness screenshot counts", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("layout-check counts", StringComparison.OrdinalIgnoreCase)
        ...
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("workflow stages", StringComparison.OrdinalIgnoreCase)
        ...
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            ...
            && evidence.Contains("required-review status", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            evidence.Contains("visual-smoke-manifest.json route audits", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("visual-smoke-evidence-report.json", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            evidence.Contains("positive byte size", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("canonical sha256:<64 lowercase hex> checksum", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
            evidence.Contains("match a retained PNG file", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("file name", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("byte size", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("sha256 checksum", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["frontend-accountant-workbench"].CurrentEvidence, evidence =>
        ...
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("explicit accepted scenario scope cells", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("source-law evidence", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("workbench journey", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("exact yes decision-question cells", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("exact accepted evidence cells", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("Qualified-accountant route decision-question cells", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("reject accepted-style ambiguous text", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("route-specific", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("accountant-workbench evidence anchors", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
        ...
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
            evidence.Contains("workbench evidence reference", StringComparison.OrdinalIgnoreCase)
            && evidence.Contains("accountant-workbench-evidence-report.json", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(scores["backend-statutory-accounting-engine"].CurrentEvidence, evidence =>
